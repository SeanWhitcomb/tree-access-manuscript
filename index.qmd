...
I used the Mann-Whitney *U* test to determine whether TTCC across all block groups in the city is affected by the presence of public parks. The median TTCC of the 176 block groups with parks was compared to the median TTCC of the 841 block groups without parks, and the *U* statistic was calculated.

I performed a Wilcoxon signed-rank test on block groups that contain public parks to determine whether parks contribute to total tree canopy cover within those block groups.
...
The Gaussian two-step floating catchment area (G2SFCA) method was used to determine Phoenix residents' spatial access to tree canopy cover in public parks. This method accounts for both the supply of public amenities and demand on those amenities when calculating accessibility. The population of areas surrounding an amenity represents the demand on the amenity [@hu2020]. In other words, more people living near a park leads to greater competition for the park's amenities, including tree canopy cover.
...
ggplot(parks_plot, aes(x = park_type, 
  ...
  labs(x = "Park type", y = "Tree canopy area per park, log scale (ha, log scale)") +
  theme_minimal() +
...
Public park access studies often use park centroids as destination points [@dai2011; @hu2020; @peng2025]. However, this method can bias the results against large parks since users may have to walk long distances from a park entrance to the centroid [@zhang2023a]. This study used actual park entrances, determined by visual examination in QGIS, as destination locations for parks larger than 2 ha. For smaller parks, the centroid was used since travel distance would be minimally affected by the access point location.
...
Previous studies on park access tend to rely on arbitrary catchment sizes, typically in the range of 0.5 to 1 km, based on the assumption that people will only walk a short distance to visit parks [@elderbrock2024; @lara-valencia2018; @so2016] . However, Phoenix is a uniquely car-dependent city [@saeidizand2024], and Kim et al. (2025) have shown that Phoenix residents are willing to travel farther to access larger parks with more amenities [@kim2025]. Therefore, rather than rely on a single catchment size ($d_0$) for all parks, $d_0$ was determined by the service area radius assigned by the City of Phoenix to each park type (pocket, linear, neighborhood, community, or regional) [@cityofphoenixa]. Pocket and linear parks were assigned a threshold of 0.8 km (0.5 mi), neighborhood parks 1.6 km (1 mi), community parks 4.8 km (3 mi), and regional parks 8.0 km (5 mi). This aligns the park catchment values with the park accessibility goals of the City of Phoenix.

While the focus of this study is on Phoenix residents' access to parks, people from nearby cities are free to visit Phoenix parks, thus diluting the quantity of park tree canopy available to Phoenix residents. Therefore, the populations of `r nrow(park_bg_pw) - nrow(phx_bg_all)` block groups from surrounding cities that lie within park catchment areas were also included in the calculation of the supply/demand ratios ($R_j$ and $R_l$).
...
Demographic and environmental variables often exhibit spatial autocorrelation, meaning the residuals in a given areal unit are likely to be similar to their surrounding units. The presence of spatial autocorrelation violates the assumption of independent residuals required for for OLS regression, necessitating the use of spatially aware regression models [@anselin1991]. The global Moran's *I* index was calculated to measure the overall spatial autocorrelation of the residuals of both OLS regression models using the following formula:
...
Block groups were classified according to RTCC and PTAS values. Because PTAS already captures park tree canopy as its supply-side variable, using TTCC as an outcome measure would essentially count the same trees twice: once in the block group tree canopy access and once in the park accessibility axis of the typology [see @fossa2023]. Use of RTCC avoids the duplication issue.

Block groups with an RTCC value of 15% or higher were classified as high RTCC category, following American Forests' recommendation that desert cities should maintain a 15% tree canopy cover to achieve tree equity [@cityofphoenix]; the remainder were classified as low RTCC. In the absence of comparable policy guidelines for park tree access, block groups with a PTAS at or above the median were classified as high PTAS, and those below the median as low PTAS. The two factors were then combined to create four tree canopy access categories: a) high RTCC/high PTAS (HH), b) high RTCC/low PTAS (HL), c) low RTCC/high PTAS (LH), or d) low RTCC/low PTAS (LL). Multinomial logistic regression was used to examine whether the percent Hispanic population, percent Black population, or ADI rank predicted a block group's membership in each tree canopy access category relative to the high RTCC/high PTAS (HH) condition [@sánchez-díaz2025].
...
This comparision does not indicate whether parks add canopy within their own block groups, however. To test this, I compared each park-containing block group's TTCC to its RTCC directly. Of the `r nrow(phx_bg_parks)` block groups that contain parks, the median TTCC was `r ttcc_med`%, while RTCC was lower at `r rtcc_med`% (@tbl-ttcc-rtcc). This difference was statistically significant (Wilcoxon signed-rank test, *V* = `r wilcox_v`, *p* \< `r wilcox_p`), confirming that parks meaningfully contribute to tree canopy cover when they are present in a neighborhood. Interestingly, although the presence of parks had an overall positive impact on TTCC, `r negative_diff$n_neg` (`r negative_diff$pct_neg`%) of the park-containing block groups saw a reduction in TTCC compared to RTCC. This occurs when a park with low tree canopy cover is located in a block group with a higher canopy cover, thus reducing the neighborhood's total tree canopy percentage.
...
Given that RTCC is inequitably distributed in Phoenix, while PTAS is higher in socially vulnerable neighborhoods, public parks do seem to serve as tree canopy refuges in areas where this canopy is arguably needed the most. Compared to block groups with abundant RTCC and PTAS, tree canopy deserts (block groups with low RTCC and low PTAS) are significantly more likely to be socioeconomically disadvantaged and marginally more likely to have large Black populations. However, block groups in which parks serve as tree canopy refuges (those with low RTCC and high PTAS) are also more likely to have larger Black populations and higher deprivation. This indicates that tree canopy cover within Phoenix public parks is well-situated to at least partially compensate for a lack of residential tree canopy in areas that are most vulnerable to urban heat island effects [@zhu2022]. A concerted effort should be made by the City of Phoenix to establish new parks and increase tree cover within existing parks in the 37.8% of block groups that qualify as true tree canopy deserts.

The tree canopy access patterns found here both echo and diverge from Elderbrock et al.'s (2024) study of tree canopy equity in Portland, Oregon. Like this study, those authors found less residential tree canopy in block groups with larger Black, Hispanic, and low-income populations. Unlike this study, however, they found no compensatory pattern in park tree canopy access, which was unrelated to race or income and was actually *lower*, not higher, for Hispanic residents [@elderbrock2024]. This divergence may be due to a difference in function between Portland and Phoenix parks, or it may be methodological. Elderbrock et al. measured park access with a simple 800-m buffer around block group boundaries, a binary measure that does not weight nearby parks more heavily than distant ones or account for differences in tree-canopy demand based on block group population. The Gaussian two-step floating catchment area (G2SFCA) method used here incorporates both of these factors, along with distance measured from actual park entrances, which Zhang et al. (2023) found reduces the underestimations of accessibility that centroid- or perimeter-based distances can produce [@zhang2023].

### Limitations and future research {#sec-disc-limit}

One limitation of this research is the two-dimensional land cover imagery used to estimate tree canopy cover. Although the 0.3-m resolution of this imagery was higher than that used in similar studies [@wang2024; @zhou2013] , it cannot capture factors like tree height or shade density, which will impact how residents experience the tree shade available in their neighborhoods and parks [@colter2019] . High-quality LiDAR imagery could improve the analysis by providing detailed three-dimensional models of the trees.
...
Phoenix, Arizona has an urgent need for greater tree canopy cover, especially in areas where residents are most vulnerable to extreme heat. This study demonstrated that Black residents, Hispanic residents, and residents with lower socioeconomic status are more likely to live in block groups with low residential tree canopy cover. However, these disparities are not reflected in access to public park tree canopy. Hispanic residents have equal access to park tree canopy compared to non-Hispanic residents, while Black residents and those in high-deprivation areas have greater access to public park trees. Combining measures of residential tree canopy cover with park tree access revealed that there are many block groups in Phoenix that can be considered tree canopy deserts, and these areas have more Black and low-SES residents. However, low-canopy block groups with nearby parks that serve as tree canopy refuges are also more likely to be located in areas with large Black populations and greater deprivation. These findings suggest that the City of Phoenix should focus on creating new parks and increasing park tree canopy in tree desert neighborhoods, allowing these parks to develop into tree canopy refuges where they are need the most.

## Acknowledgements {#sec-acknowledge}

I am grateful to Jake Simon at American Forests for providing access to the land cover data upon which this study was based. I also thank Phoenix Parks and Recreation for allowing open access to their data and for creating a park system that provides innumerable benefits to the people of Phoenix.
