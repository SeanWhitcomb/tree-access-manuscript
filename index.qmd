...
Traditional park access research often only addresses the supply side of access by quantifying park number, area, or amenities [@kim2025; @nesbitt2019; @winkler2024] while ignoring the demand side, or the number of people competing for access to the parks. Other studies incorporate the population factor by simply dividing the total park area by the population within a given catchment [@lara-valencia2018]. Gravity-based methods calculate park access by incorporating distance and park coverage, giving greater weight to nearby parks [@park2020]. One such gravity-based approach is the Gaussian two-factor catchment area (G2SFCA) method, which incorporates a park supply factor (e.g., total area or amenities), a demand factor (the population of residents with access to the parks), and a travel distance metric that weights closer parks more heavily than distant parks [@zhang_assessing_2023].

Residential tree canopy cover provided by trees in neighborhoods and commercial areas likely has a more direct impact on residents' lives than the tree canopy in public parks since residential trees shade people's homes, workplaces, and travel routes. When parks in disadvantaged neighborhoods have low tree canopy cover, this may perpetuate the overall inequitable tree distribution in cities. For example, Fossa et al. (2023) found that parks have greater tree cover and overall greenness compared to surrounding neighborhoods in New York City, NY, Chicago, IL, and Los Angeles, CA. However, areas with larger Black, Hispanic, and socioeconomically deprived populations contained parks with lower tree canopy cover [@fossa2023]. Park and Guldmann (2020) found that total tree canopy and park access were positively correlated in metro Columbus, Ohio and Atlanta, Georgia, indicating that park tree canopy reinforces the overall tree canopy disparities found in both cities [@park2020].

In other cases, parks may serve as tree canopy refuges in low-canopy residential areas, allowing people to temporarily enjoy the benefits of trees that may be lacking in their daily lives. A 2013 study by Zhou and Kim investigated neighborhood tree canopy disparities and walking-distance access to parks in six Illinois cities. They found that in four of the cities, a higher proportion of Black residents was associated with lower neighborhood tree canopy cover, but there were no disparities in access to parks. However, Zhou and Kim did not quantify the tree canopy cover within the parks, so it is unclear whether the park trees compensate for the lack of trees in the surrounding neighborhoods [@zhou2013].
...
park_types |> 
  ...
  cols_width(
    `Park type` ~ px(120),
    `Typical amenities` ~ px(220),
    `n` ~ px(30),
    `Size (ha)` ~ px(60),
    `Service area radius (km)` ~ px(120)
  ) |> 
  cols_align("center",
             columns = 3:4) |> 
  tab_style(
