...
mn_table |>
  ...
  tab_footnote(md("† *p* < 0.1, * *p* < 0.05, ** *p* < 0.01, *** *p* < 0.001")) |>
...
This divergence may be due to a difference in function between Portland and Phoenix parks, or it may be methodological. Elderbrock et al. measured park access with a simple 800-m buffer around block group boundaries, a binary measure that does not weight nearby parks more heavily than distant ones or account for differences in tree-canopy demand based on block group population. The Gaussian two-step floating catchment area (G2SFCA) method used here incorporates both of these factors, along with distance measured from actual park boundaries, which Zhang et al. (2023) found reduces the underestimations of accessibility that centroid- or perimeter-based distances can produce [@zhang_assessing_2023].
