...
Discuss use of ADI vs. median income

Given that multiple studies have failed to uncover significant inequities in park access in Phoenix, it is reasonable to ask whether public parks may act as tree canopy refuges that partially compensate for lack of tree cover in neighborhoods with high minority populations and low socioeconomic status. However, no research to date has addressed this possibility. This study attempts to fill this research gap by answering the following questions:
...
![The effect of public park tree canopy cover on total tree canopy cover within a block group: a) Total tree canopy cover (TTCC) = 11.7%, b) Residential tree canopy cover (RTCC) = 8.2%](images/fig-bg-tree-cover.png){#fig-tree-cover fig-alt="Map showing a block group with 11.7% tree canopy cover. The tree canopy reduces to 8.2% when a large park is clipped out of the block group."}
...
![a) Residential tree canopy cover (%) and b) park tree access score](images/map_rtcc_ptas.png){#fig-rtcc-ptas-map fig-alt="Residential tree canopy cover is highest in the central and northeastern parts of Phoenix. Park tree access score is highest in the central part."}

![a) Hispanic population (%), b) Black population (%), and c) ADI national percentile rank](images/map_demographics.png){#fig-bg-demographics fig-alt="Hispanic population is highest in the southwest of Phoenix; Black population is highest in the southern part with a small pocket in the north; ADI rank is highest in the western part with a high cluster in the north"}
...
In the OLS model, the percentage of Hispanic residents in a block group is not significantly associated with the square root of PTAS, but there was a significant positive association between the square root of PTAS and both Black resident percentage (*p* \< 0.05) and ADI rank (*p* \< 0.001; see @tbl-regress).
