...
As seen in the Phoenix studies cited above, research on park access tends to use a simple Euclidean distance metric to determine accessibility, but this method does not account for the actual distance that residents must travel to visit parks. The travel distance from a residential area to a public amenity can vary greatly from the Euclidean distance depending on the layout of the road network [@li2019]. Additionally, most studies consider "park access" to be a binary variable, where a resident has complete access to any parks within a given catchment area and no access to parks outside of the catchment area. However, this approach does not capture the reality of park visitation–people are more likely to visit nearby parks, but may still travel a greater distance to visit larger parks. For example, Kim et al. (2025) found that Phoenix residents tend to favor local parks within 1 km of their neighborhood, but the likelihood that they will travel beyond 1 km to visit larger amenity-rich parks increases as the size of the local parks decreases [@kim2025].

Public green space access research often only addresses the supply side of access by quantifying park number, acreage, or amenities within a residential catchment area while ignoring the demand side, or the number of people competing for access to the parks [@kim2025; @nesbitt2019; @winkler2024]. Other studies incorporate the population factor by calculating park area per capita, dividing the total park area by the population within a catchment [@kabisch2014; @lara-valencia2018]. However, this approach does not consider travel distance to parks; residents are more likely to visit nearby public amenities, and travel friction increases as a function of distance [@yang2022]. Gravity-based access measures are increasingly being used in public green space access research because they incorporate a supply factor, a demand factor, and a travel distance metric that weights closer amenities more heavily than distant amenities [@luo2003; @zhu_modeling_2025; @zhang_assessing_2023]. The Gaussian two-step floating catchment area (G2SFCA) method is one such gravity-based measure that allows for variable catchment sizes, incorporates population size within the catchments, and applies a travel penalty that increases with distance from residential areas to parks [@zhang2019].

Residential tree canopy cover provided by trees in neighborhoods and commercial areas likely has a more direct impact on residents' lives than the tree canopy in public parks since residential trees shade people's homes, workplaces, and travel routes. When parks in disadvantaged neighborhoods have low tree canopy cover, this may perpetuate the overall inequitable tree distribution in cities. For example, Fossa et al. (2023) found that parks have greater tree cover and overall greenness compared to surrounding neighborhoods in New York City, NY, Chicago, IL, and Los Angeles, CA. In general, areas with larger Black, Hispanic, and socioeconomically deprived populations contained parks with lower tree canopy cover [@fossa2023]. Park and Guldmann (2020) found that total tree canopy and access to park acreage were positively correlated in metro Columbus, Ohio and Atlanta, Georgia, indicating that neighborhoods with low tree cover also had lower access to public green space [@park2020]. But the approaches used in these two studies did not specifically isolate park tree canopy from total tree canopy, so it is unknown to what extent park trees may compensate for a lack of residential tree canopy.

Two studies have found indications that parks may serve as tree canopy refuges in disadvantaged areas with low total tree canopy cover. A 2013 study by Zhou and Kim investigated neighborhood tree canopy disparities and access to parks in six Illinois cities. They found that in four of the cities, a higher proportion of Black residents was associated with lower neighborhood tree canopy cover, but there were no disparities in access to parks [@zhou2013]. Heo and Bell (2023) studied green space equity in three eastern U.S. cities and found that tree canopy was negatively associated with percent people of color and socioeconomic deprivation, while the demographic factors were positively associated with park access [@heo2023]. The results of these two studies suggest that parks may sometimes serve as tree canopy refuges in disadvantaged neighborhoods; however, the authors only quantified general park access, not the actual tree canopy cover within the parks.
...
This research expands on existing studies of tree canopy cover in Phoenix by isolating residential tree canopy from the canopy cover provided by public park trees to avoid double counting of park trees in the analysis. It also improves on the analyses of park tree canopy access by using the G2SFCA method based on road network distance to capture a more realistic picture of accessibility compared to traditional Euclidean distance methods that ignore population-driven demand on tree canopy cover. And it uses spatially aware regression methods to analyze associations between tree canopy cover and demographic factors to ensure that spatial autocorrelation does not overly influence the models.
...
Block groups with an RTCC value of 15% or higher were classified as high RTCC category, following American Forests' recommendation that desert cities should maintain a 15% tree canopy cover to achieve tree equity [@cityofphoenix]; the remainder were classified as low RTCC. In the absence of comparable policy guidelines for park tree access, block groups with a PTAS at or above the median were classified as high PTAS, and those below the median as low PTAS. The two factors were then combined to create four tree canopy access categories: a) high RTCC/high PTAS (HH), b) high RTCC/low PTAS (HL), c) low RTCC/high PTAS (LH), or d) low RTCC/low PTAS (LL).

Multinomial logistic regression was used to examine whether the percent Hispanic population, percent Black population, or ADI rank predicted a block group's membership in each tree canopy access category relative to the high RTCC/high PTAS (HH) condition [@sánchez-díaz2025]. Given that spatial autocorrelation was detected in the residuals of both the RTCC and PTAS regression residuals, it was expected that the residuals of the tree access category logistic regression model might exhibit similar spatial dependence. However, global Moran's *I* cannot be calculated directly on the residuals of a multinomial logistic model, since the outcome comprises four unordered categories rather than a single continuous or binary residual vector [@anselin1991]. To address this, each tree canopy access category (HH, HL, LH, LL) was recast as a binary outcome (one-vs-rest) and modeled separately via logistic regression on the same three demographic predictors. Global Moran's *I* was then calculated on the residuals of each binary model to test for spatial autocorrelation.
...
@tbl-bin-regress reports the β coefficients for each tree canopy access binary logistic models before and after spatial filtering. Both the HH and HL categories were negatively associated with ADI national percentile rank, while the LH and LL categories were positively associated with ADI rank. (CHECK THIS!!!! No p stars in the table) These associations were significant both before and after spatial filtering, indicating that the relationship between ADI and canopy access category is robust to the presence of spatial autocorrelation in the model residuals. Significance levels for most other predictor β terms also remained stable after filtering, with two exceptions: for the HH category, the negative associations with Hispanic population and Black population moved from marginal significance (*p* \< 0.1) to significance at *p* \< 0.05; and for HL, the negative association with Hispanic population shifted from non-significant to significant at *p* \< 0.05, while the association with Black population strengthened from *p* \< 0.05 to *p* \< 0.01. CHECK RESULTS FOR THIS SECTION AND WRITE TO BE MORE CLEAR

-----------

Across the four canopy-access typology categories (reference: HH), coefficient signs, magnitudes, and significance patterns remain largely stable between the unfiltered and spatially filtered (ESF) models, supporting the robustness of the primary multinomial findings. ADI rank is the most consistent predictor: it is negatively associated with membership in HH (β = -0.044, p \< .001 unfiltered; β = -0.039, p \< .001 filtered) and HL (β = -0.065; β = -0.068, both p \< .001) relative to reference, and positively associated with LH (β = 0.032; β = 0.038, both p \< .001), with virtually no attenuation after spatial filtering — indicating this relationship is not an artifact of spatial autocorrelation. Racial composition effects show a similar pattern of stability with two notable exceptions. In HH, both Hispanic and Black population share strengthen from marginal to conventional significance after filtering (Hispanic: p = .062 to p = .020; Black: p = .078 to p = .042), and in HL, Black population share strengthens from p = .024 to p = .0036 while Hispanic population share newly reaches significance (p = .135 to p = .0068). The clearest divergence is in LH, where the positive association with Hispanic population share found in the unfiltered model (β = 0.007, p = .027) loses significance after spatial filtering (β = 0.006, p = .118), while the Black population share effect in this category remains significant in both specifications (p = .0084; p = .0043). No demographic or deprivation predictor reaches significance for LL in either model. Given that the ESF procedure is documented elsewhere as reducing residual spatial autocorrelation by 76–85% across categories, the overall stability of coefficients suggests the multinomial results are not substantially confounded by unmodeled spatial structure — with the LH Hispanic effect representing the one relationship whose evidentiary strength depends on whether spatial autocorrelation is explicitly filtered, a nuance that should be flagged explicitly given the analogous Hispanic-effect divergence already noted between the continuous PTAS and typology models.
...
Given that RTCC is inequitably distributed in Phoenix, while PTAS is higher in socially vulnerable neighborhoods, public parks do seem to serve as tree canopy refuges in areas where this canopy is arguably needed the most. Compared to block groups with abundant RTCC and PTAS, tree canopy deserts (block groups with low RTCC and low PTAS) are much more likely to be socioeconomically disadvantaged and marginally more likely to have large Black populations. However, block groups in which parks serve as tree canopy refuges (those with low RTCC and high PTAS) are also more likely to have large Black populations and high deprivation. This indicates that tree canopy cover within Phoenix public parks is well-situated to at least partially compensate for a lack of residential tree canopy in areas that are most vulnerable to urban heat island effects [@zhu2022]. A concerted effort should be made by the City of Phoenix to increase park tree cover in the 37.8% of block groups that qualify as true tree canopy deserts.

The tree canopy access patterns found here both echo and diverge from Elderbrock et al.'s (2024) study of tree canopy equity in Portland, Oregon. Like this study, the authors found less residential tree canopy in block groups with larger Black, Hispanic, and low-income populations. Unlike this study, however, they found no compensatory pattern in park tree canopy access, which was unrelated to race or income and was actually *lower*, not higher for Hispanic residents [@elderbrock2024].

This divergence may be due to a difference in function between Portland and Phoenix parks, or it may be methodological. Elderbrock et al. measured park access with a simple 800-m buffer around block group boundaries, a binary measure that does not weight nearby parks more heavily than distant ones or account for differences in tree-canopy demand based on block group population. The Gaussian two-step floating catchment area (G2SFCA) method used here incorporates both of these factors, along with distance measured from actual park boundaries, which Zhang et al. (2023) found reduces the underestimations of accessibility that centroid- or perimeter-based distances can produce [@zhang2023].
...
Phoenix, Arizona has an urgent need for greater tree canopy cover, especially in areas where residents are most vulnerable to extreme heat. This study demonstrated that Black residents, Hispanic residents, and residents with lower socioeconomic status (SES) are more likely to live in block groups with low residential tree canopy cover. However, these disparities are not reflected in access to public park tree canopy. Hispanic residents have equal access to park tree canopy compared to non-Hispanic residents, while Black residents and those in high-deprivation areas have greater access to public park trees. Combining measures of residential tree canopy cover with park tree access revealed that there are many block groups in Phoenix that can be considered tree deserts, and these areas have more Black and low-SES residents. However, low-canopy block groups with nearby parks that serve as tree canopy refuges are also more likely to be located in areas with large Black populations and greater deprivation. These findings suggest that the City of Phoenix should focus on creating new parks and increasing park tree canopy in tree desert neighborhoods, allowing these parks to develop into tree canopy refuges where they are need the most.

## Acknowledgements {#sec-acknowledge}

I am grateful to Jake Simon at American Forests for providing access to the land cover data upon which this study was based. I would also like to thank Phoenix Parks and Recreation for allowing open access to their data and for creating a park system that provides innumerable benefits to the people of Phoenix.
